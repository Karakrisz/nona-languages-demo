            ...
            if (strtotime($currentDate) == strtotime(date("Y-m-d"))) {
                echo '
								<div class="calendar__day today" onclick="getEvents(\'' . $currentDate . '\');">
									...
									<span class="calendar__task calendar__task--today">' . $eventNum . ' Tanóra</span>
								</div>
							';
            } elseif ($eventNum > 0) {
                echo '
								<div class="calendar__day event" onclick="getEvents(\'' . $currentDate . '\');">
									<span class="calendar__date">' . $dayCount . '</span>
									<span class="calendar__task">' . $eventNum . ' Tanóra</span>
								</div>
							';
            } else {
                echo '
...
function getEvents($date = '')
{
    ...
    global $db;
    $result = $db->query("SELECT title,created,modified,status FROM events WHERE date = '" . $date . "'");
    if ($result->num_rows > 0) {
        $eventListHTML .= '<ul class="sidebar__list">';
        $eventListHTML .= '<li class="sidebar__list-item sidebar__list-item--complete">Tanóra</li>';
        $i = 0;
        while ($row = $result->fetch_assoc()) {
            $i++;
            $eventListHTML .= '<li class="sidebar__list-item"><span class="list-item__time">' . $i . '.</span>' . $row['title'] . '</li>';
            $eventListHTML .= '<li class="sidebar__list-item"><span class="list-item__time"></span>' . $row['created'] .= ' - ' . $row['modified'] . '<li>';
            $eventListHTML .= $row['status'] == 0 ? 'Elfogadásra vár' : 'Elfogadva';
        }
        $eventListHTML .= '</ul>';
    }
    echo $eventListHTML;
